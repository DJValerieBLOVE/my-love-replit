...
- **Color Scheme**: Primary brand color is Purple (#6600ff) used for links, accents, and progress indicators ONLY. All action buttons use bg-foreground (dark charcoal), NOT purple. Button hover: white background with light gray border (#E5E5E5). A purple gradient from #6600ff to #9900ff (or #cc00ff) is used for aesthetic elements. Pink (#eb00a8) is exclusively reserved for the GOD/LOVE dimension and not used as a brand accent. All colors are solid hex values, no opacity.
- **Tags/Badges**: All tags and badges use light gray outline style: white background, border-gray-200, text-muted-foreground. No colored badge backgrounds. No purple highlight on dropdowns - use bg-gray-50 for hover/selected states.
- **Tabs/Bubbles**: Category tabs use rounded-full pill/bubble style matching Feed page (bg-foreground when active, bg-white with border-gray-200 when inactive). NOT shadcn Tabs component.
- **Dropdowns**: Select triggers use h-9 text-sm to match search bar height. Items use bg-gray-50 for hover/focus, not purple.
- **Terminology**: "Experiments" is the primary term for learning content. Buttons say "Start Experiment" / "View Experiment" (not "View Course").
- **Visual Style**: Features a purple gradient aesthetic. The homepage is structured as a "Prosperity Pyramid" with 5 areas (Health, People, Purpose, Wealth) centered around GOD. Navigation includes a left sidebar (desktop) and top header. Cards use `rounded-xs` corners with flip animations, where flipped card backs have a light background (#FAF8F5). Ghost button hover uses light purple (#F5F3FF/foreground text), never teal. All action buttons use bg-foreground (dark charcoal), NOT purple. Purple (#6600ff) is reserved for links, accents, and progress indicators only.
- **Feed Layout**: Three-column Facebook/X-style layout. Feed column (~620px) centered on viewport using CSS clamp() margin formula. Left sidebar (240px) and right sidebar (300px) are both sticky during scroll. Feed tabs (Following/Tribes/Buddies) aligned with feed column, Explore/Trending dropdown centered over right sidebar. New posts notification pill appears inline below sticky tabs, not overlapping. Header is solid white (opaque, no blur/transparency).
